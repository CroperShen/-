...
public:
	...
	virtual void operator=(const linkedlist<T>&);                 //重载赋值 使其变为复制
	virtual const bool operator==(const linkedlist<T>&) const;    //链表相等，只有在链表每一个元素都完全相同时才相等;
	virtual const bool operator!=(const linkedlist<T>&) const ;   //链表不相等;
	virtual T& operator[](int);                                   //重载下标符号，能直接像数组一样使用
	...
	link<T>& Append(const T&);             //于尾部添加一个值，返回添加值的节点
	link<T>& Append();                     //于尾部添加一个值，返回添加值的节点
	link<T>& Insert(const T& ,int);        //于给定位置添加一个值，其他值顺延，返回添加值的节点
	link<T>& Insert(int);        //于给定位置添加一个值，其他值顺延，返回添加值的节点
	link<T>& SetHead(const T&);            //改变头节点值
	link<T>& SetTail(const T&);            //改变尾节点的值
	link<T>& SetValue(int,const T&);       //改变给定位置的值
...
//---------------
template <typename T> link<T>& linkedlist<T>::Append(const T& data)          //于尾部添加一个值
{
...
template <typename T> link<T>& linkedlist<T>::Append()          //于尾部添加一个值为默认的节点
{
	link<T> *p = new link<T>;
	p->PreNode = headernode->PreNode;
	p->NextNode = headernode;
...
//----------------------
template <typename T> link<T>& linkedlist<T>::Insert(const T& data,int i)   //于给定位置添加一个值，其他值顺延
{
	link<T> *p = new link<T>(data);
	link<T> *pPre;
...
template <typename T> link<T>& linkedlist<T>::Insert(int i)   //于给定位置添加一个值为默认的节点，其他值顺延
{
	link<T> *p = new link<T>;
	link<T> *pPre;
...
template <typename T> link<T>& linkedlist<T>::SetHead(const T& t)            //改变头节点值
{
	if (size() == 0) return;
	pHead()->Value= t;
}

template <typename T> link<T>& linkedlist<T>::SetTail(const T&t)             //改变尾节点的值
{
	if (size() == 0) return;
	pTail()->Value = t;
}
template <typename T> link<T>& linkedlist<T>::SetValue(int i,const T&t)        //改变给定位置的值
{
	if (size() == 0) return;
